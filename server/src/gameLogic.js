...
const cleanupInactiveRooms = () => {
  ...
  Object.keys(rooms).forEach((roomCode) => {
    ...
    const allInactive = players.every(
      (player) => now - room[player].lastActivity > inactivityThreshold
    );
...
const startGame = (roomCode) => {
  ...
  console.log(
    `Game started in room ${roomCode} with players: ${players.join(
      ", "
    )}. Token: ${token}`
  );
};
...
const updateBoard = (roomCode, nickname, token, board) => {
  if (
    rooms[roomCode] &&
    rooms[roomCode][nickname] &&
    rooms[roomCode][nickname].token === token
  ) {
...
const checkBothPlayersReady = (roomCode) => {
  ...
  if (players) {
    ...
    if (allReady) {
      console.log(
        `Both players in room ${roomCode} are ready. Starting the game.`
      );
...
const getBoard = (roomCode, nickname, token) => {
  if (
    ...
    if (board) {
      ...
      console.log(
        `Game stage for ${nickname} in room ${roomCode} set to battle`
      );
    }
...
const attemptReconnect = (roomCode, nickname, token, ws) => {
  ...
  console.error(
    `Reconnect failed for ${nickname} in room ${roomCode}. Invalid room, nickname, or token.`
  );
  return false;
};
...
const updateVisibleOpponentBoard = (roomCode, nickname, opponentBoard) => {
  if (rooms[roomCode] && rooms[roomCode][nickname]) {
    ...
    console.log(
      `Updated visible opponent board for ${nickname} in room ${roomCode}:`
    );
...
const getCurrentTurn = (roomCode, nickname) => {
  if (rooms[roomCode] && rooms[roomCode][nickname]) {
    ...
    console.log(
      `Current turn in room ${roomCode} belongs to ${currentTurnPlayer}`
    );
...
const switchTurn = (roomCode, nickname) => {
  ...
  if (players.length < 2) {
    return null; // Возвращаем null, если в комнате только один игрок
  }
  ...
  if (nextTurnPlayer) {
    ...
    console.log(
      `Turn switched in room ${roomCode}. Now it's ${nextTurnPlayer}'s turn.`
    );
...
const shoot = (row, col, roomCode, playerNickname) => {
  ...
  const opponentNickname = Object.keys(room).find(
...
const wrappedUpdatePlayerReadyStatus = wrapWithActivityUpdate(
...
const wrappedUpdateVisibleOpponentBoard = wrapWithActivityUpdate(
